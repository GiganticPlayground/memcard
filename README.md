...
Memcard can emit a normalized record for each request/response through
[reqcast](https://github.com/GiganticPlayground/reqcast), the same middleware
Qodi uses. It is **opt-in**: analytics stay off unless a config file is present.
Enable it by pointing `REQCAST_CONFIG` at one, or by placing a
`reqcast.config.json` in the working directory.

```bash
cp examples/reqcast.config.json ./reqcast.config.json
# or
REQCAST_CONFIG=./examples/reqcast.config.json yarn dev
```

Everything about what is captured, how it is shaped, and where it goes lives in
that file — reqcast validates it at startup and fails fast if it is invalid. The
bundled example logs one record per `/v1/memcard/**` request (`/health` and
`/api-docs/**` excluded) and adds the error body on 4xx/5xx responses. Swap the
`log` sink for `file` or `amqp` to ship records elsewhere.

Two Memcard-specific notes on the example config:

- `capture.requestBody` is `false` — a `PUT` body is the player's entire state
  blob and is not useful as analytics.
- `capture.responseBody` is `true` only so error bodies can be projected on
  4xx/5xx; successful responses omit it from the record.

The `x-request-id` header (generated by `requestContextMiddleware` when absent)
becomes the record's `requestId`, so analytics records correlate with the
service's own logs. On shutdown, in-flight requests drain before the sinks are
flushed and closed.
